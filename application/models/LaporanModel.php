...
class LaporanModel extends CI_Model {
    ...
    function laporan_barang_pending($start, $end)
    {
        ...
        $this->db->join('supplier as s', 'db.id_supplier = s.id', 'LEFT');
    ...
    function laporan_data_barang($start, $end)
    {
        ...
        $this->db->select('db.*, bm.jumlah_barang, s.nama_supplier');
        $this->db->from('data_barang as db');
        $this->db->join('barang_masuk as bm', 'db.id = bm.id_barang', 'LEFT');
        $this->db->join('barang_detail as bk', 'db.id = bk.barang_id', 'LEFT');
        $this->db->join('supplier as s', 'db.id_supplier = s.id', 'LEFT');
    ...
    function laporan_data_supplier($id_supplier)
    {
        $this->db->select('db.*, s.nama_supplier, s.nama_pj, s.no_telp');
        if (!empty($id_supplier)) {
            $this->db->where('db.id_supplier', $id_supplier);
        }
    ...
    // function laporan_data_bidang($id_bidang)
    // {
    //     $this->db->select('bk.*, db.nama_barang, db.jenis_barang, b.nama_bidang, b.nama_kasi');
    //     if (!empty($id_bidang)) {
    //         $this->db->where('bk.id_bidang', $id_bidang);
    //     }  

    //     $this->db->from('barang_keluar as bk');
    //     $this->db->join('data_barang as db', 'bk.id_bidang = db.id', 'LEFT');
    //     $this->db->join('bidang as b', 'bk.id_bidang = b.id', 'LEFT');
    //     // $this->db->join('supplier as s', 'bk.id_supplier = s.id', 'LEFT');
